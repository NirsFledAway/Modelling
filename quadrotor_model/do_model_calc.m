function data = do_model_calc(t, x, uu, MAV)
%   Earth coordinates
    pn    = x(1);
    pe    = x(2);
    pd    = x(3);
...
%   angular speeds
    ...
    cache.R_g_b = R_g_b;
    [Fb, Mb, force_telemetry] = forces_moments(t,x,uu, wind, MAV, cache);
    telemetry = struct();
    telemetry.Fb = force_telemetry;
    telemetry.Mb = Mb;
...
function [Fb, Mb, telemetry] = forces_moments(t, x, uu, wind_speed, MAV, cache)
...
%     wind = R_g_b * MAV.Env.Wind_speed;
    ...
    telemetry.aerial = f_aerial_drag;
    telemetry.thrust = f_thrust;
    telemetry.gravity = f_gravity;
...
%     C_0 = 0.715 / (0.5*rho_0*Va_0^2);  % if alpha = 0, beta = 0;
    ...
    alpha = min(abs(alpha), 20)*sign(alpha);
    if alpha >= 0 && alpha <= 20
...
%     C_D_alpha = F_0 / (0.5*rho_0*Va_0^2);
    ...
    b = min(abs(b), 20)*sign(b);       %ограничение по углу у альфы 20 потому что дальше пиздец растёт 
